...
//                                ->check(CLI::ExistingFile);
   ...
   string transaction_perm1;
   string proposed_transaction1;
   string proposed_contract1;
   string proposed_action1;
   ...
   propose_action->add_option("data", proposed_transaction, localized("The JSON string or filename defining the action to propose"))->required();
    ...
    propose_action->add_option("trx_permissions1", transaction_perm1, localized("The JSON string or filename defining transaction permissions"))->required();
   propose_action->add_option("contract1", proposed_contract1, localized("contract to which deferred transaction should be delivered"))->required();
   propose_action->add_option("action", proposed_action1, localized("action of deferred transaction"))->required();
   propose_action->add_option("data1", proposed_transaction1, localized("The JSON string or filename defining the action to propose"))->required();
   ...
   propose_action->set_callback([&] {
      ...
      try {
         transaction_perm_var1 = json_from_file_or_string(transaction_perm1);
      } EOS_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse permissions JSON '${data}'", ("data",transaction_perm1))
      fc::variant trx_var1;
      try {
         trx_var1 = json_from_file_or_string(proposed_transaction1);
      } EOS_RETHROW_EXCEPTIONS(transaction_type_exception, "Fail to parse transaction JSON '${data}'", ("data",proposed_transaction))
      transaction proposed_trx1 = trx_var1.as<transaction>();
      bytes proposed_trx_serialized1 = variant_to_bin( proposed_contract1, proposed_action1, trx_var1 );
      ...
      vector<permission_level> trxperm1;
      try {
         trxperm1 = transaction_perm_var1.as<vector<permission_level>>();
      } EOS_RETHROW_EXCEPTIONS(transaction_type_exception, "Wrong transaction permissions format: '${data}'", ("data",transaction_perm_var1));
      ...
      trx.actions = { chain::action(trxperm, name(proposed_contract), name(proposed_action), proposed_trx_serialized),
        chain::action(trxperm1, name(proposed_contract1), name(proposed_action1), proposed_trx_serialized1) };
